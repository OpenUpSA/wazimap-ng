...
class CustomFKQuerySet(models.QuerySet):
    ...
    def get_category_queryset(self, user: User) -> QuerySet:
    ...
    def get_profile_queryset(self, user: User) -> QuerySet:
        return permissions.get_objects_for_user(
            user, Profile, include_public=False
        )

    def get_theme_queryset(self, user: User) -> QuerySet:
        profiles = permissions.get_objects_for_user(
            user, Profile, include_public=False
        )
        return self.filter(profile__in=profiles)

    def get_indicatorcategory_queryset(self, user: User) -> QuerySet:
        profiles = permissions.get_objects_for_user(
            user, Profile, include_public=False
        )
        return self.filter(profile__in=profiles)

    def get_indicatorsubcategory_queryset(self, user: User) -> QuerySet:
        profiles = permissions.get_objects_for_user(
            user, Profile, include_public=False
        )
        return self.filter(category__profile__in=profiles)

    def get_indicator_queryset(self, user: User) -> QuerySet:
        datasets = permissions.get_objects_for_user(user, Dataset)
        return self.filter(dataset__in=datasets)

    def get_geographyhierarchy_queryset(self, user: User) -> QuerySet:
    ...
    def get_dataset_queryset(self, user: User) -> QuerySet:
        return permissions.get_objects_for_user(user, Dataset)

    def get_page_queryset(self, user: User) -> QuerySet:
        profiles = permissions.get_objects_for_user(
            user, Profile, include_public=False
        )
        return self.filter(profile__in=profiles)
