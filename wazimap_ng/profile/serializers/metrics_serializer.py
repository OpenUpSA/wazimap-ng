...
def absolute_value(profile_key_metric, geography, versions):
...
def subindicator(profile_key_metric, geography, versions):
    indicator_data = get_data_for_key_metric_and_geographies(profile_key_metric, [geography])
    if indicator_data != None:
        return MetricCalculator.subindicator(indicator_data, profile_key_metric, geography)
    return None

def sibling(profile_key_metric, geography, versions):
    siblings = geography.get_siblings().filter(versions__in=versions)
...
def MetricsSerializer(profile, geography, versions):
    out_js = {}
    ...
    for profile_key_metric in profile_key_metrics:
        ...
        val = method(profile_key_metric, geography, versions)
        if val is not None:
            js = {
